...
- Fix the SwiftLint config so it lints the `Package.swift` manifests (the `.git`-suffix rule already has a live violation in `HatGame-Networking/Package.swift`), and update `README.md`/`DEVELOPMENT.md` off `master` and iOS-only.
